...
class LotteryApp:
    def __init__(self, root):
        ...
        # 后端集成相关
        self.backend_enabled = tk.BooleanVar(value=False)
        self.backend_base_url = tk.StringVar(value="http://127.0.0.1:8000")
        self.backend_participant_total = tk.IntVar(value=0)
        self.last_fairness = {"seed": None, "hash_chain": None, "prev_chain": None}
        # 聚光灯已移除，仅保留彩带动画
        ...
        # 后端模式开关
        backend_frame = ttk.Frame(self.control_panel, style='Panel.TFrame')
        backend_frame.pack(padx=12, pady=(0, 18), anchor='w')
        ttk.Label(backend_frame, text="后端模式", style='Info.TLabel').pack(side=tk.LEFT, padx=(0, 10))
        self.backend_switch = ToggleSwitch(backend_frame, self.backend_enabled, command=self.on_backend_toggle,
                           on_color=SUCCESS_COLOR, off_color="#555", knob_color="#fff")
        self.backend_switch.pack(side=tk.LEFT)
        # 后端地址输入
        ttk.Label(self.control_panel, text="后端URL", style='Info.TLabel').pack(anchor='w', padx=12, pady=(0, 4))
        self.backend_entry = ttk.Entry(self.control_panel, textvariable=self.backend_base_url)
        self.backend_entry.pack(padx=12, pady=(0, 10), anchor='w', fill=tk.X)
        # 刷新参与者按钮与统计
        self.refresh_button = ttk.Button(self.control_panel, text="刷新参与者", command=self.refresh_participants, style='Neutral.TButton')
        self.refresh_button.pack(fill=tk.X, padx=12, pady=(0, 10))
        self.participant_label = ttk.Label(self.control_panel, text="参与者: 0", style='Info.TLabel')
        self.participant_label.pack(anchor='w', padx=12, pady=(0, 4))

        # 公平性信息折叠面板
        self.fairness_open = tk.BooleanVar(value=False)
        fairness_header = ttk.Frame(self.control_panel, style='Panel.TFrame')
        fairness_header.pack(fill=tk.X, padx=12, pady=(8,4))
        ttk.Button(fairness_header, text="公平性(展开/折叠)", command=self.toggle_fairness_panel, style='Neutral.TButton').pack(side=tk.LEFT)
        self.fairness_panel = ttk.Frame(self.control_panel, style='Panel.TFrame')
        # 内部内容
        self.fair_seed_label = ttk.Label(self.fairness_panel, text="Seed: -", style='Info.TLabel')
        self.fair_prev_label = ttk.Label(self.fairness_panel, text="PrevChain: -", style='Info.TLabel')
        self.fair_chain_label = ttk.Label(self.fairness_panel, text="HashChain: -", style='Info.TLabel')
        self.fair_seed_label.pack(anchor='w', pady=2)
        self.fair_prev_label.pack(anchor='w', pady=2)
        self.fair_chain_label.pack(anchor='w', pady=2)
        ttk.Button(self.fairness_panel, text="复制公平性信息", command=self.copy_fairness_info, style='Neutral.TButton').pack(fill=tk.X, pady=(6,4))
    ...
    def finalize_winner(self, number):
        # 如果后端模式开启，实际中奖号码由后端决定（公平性依赖后端seed+hash链）
        if self.backend_enabled.get():
            backend_number = self.call_backend_draw()
            if backend_number is not None:
                number = backend_number
        self.winner = number
    ...
    def on_backend_toggle(self):
        if self.backend_enabled.get():
            self.info_label.config(text="后端模式已开启")
        else:
            self.info_label.config(text="后端模式已关闭")

    def backend_request(self, path: str, method: str = "GET", data: dict | None = None, timeout: float = 5.0):
        url = self.backend_base_url.get().rstrip('/') + path
    ...
    def refresh_participants(self):
        if not self.backend_enabled.get():
            self.info_label.config(text="后端未开启")
            return
        data, err = self.backend_request("/api/participants?skip=0&limit=10000")
        if err:
            self.info_label.config(text=f"刷新失败: {err}")
            return
        total = data.get('total', 0) if isinstance(data, dict) else 0
        self.backend_participant_total.set(total)
        self.participant_label.config(text=f"参与者: {total}")
        self.info_label.config(text="刷新成功")

    def call_backend_draw(self):
        prize = self.current_prize.get()
        payload = {"prize_level": prize, "count": 1}
        data, err = self.backend_request("/api/draw", method="POST", data=payload)
        if err or not data:
            self.info_label.config(text=f"后端抽奖失败: {err or '未知'}，使用本地号码")
            return None
        session = data.get('session') or {}
        winners = data.get('winners')
        if not winners:
            self.info_label.config(text="后端返回空结果，使用本地号码")
            return None
        backend_number = winners[0].get('number')
        if backend_number is None:
            self.info_label.config(text="后端结果缺少号码字段")
            return None
        self.info_label.config(text="后端已确认中奖")
        # 保存公平性信息
        self.last_fairness['seed'] = session.get('seed')
        self.last_fairness['hash_chain'] = session.get('hash_chain')
        self.last_fairness['prev_chain'] = data.get('prev_chain')
        self.update_fairness_panel()
        return backend_number

    def toggle_fairness_panel(self):
        if not self.fairness_open.get():
            # 展开
            self.fairness_panel.pack(fill=tk.X, padx=12, pady=(0,8))
            self.fairness_open.set(True)
            self.update_fairness_panel()
        else:
            # 折叠
            self.fairness_panel.pack_forget()
            self.fairness_open.set(False)

    def update_fairness_panel(self):
        seed = self.last_fairness.get('seed') or '-'
        prev_chain = self.last_fairness.get('prev_chain') or '-'
        chain = self.last_fairness.get('hash_chain') or '-'
        self.fair_seed_label.config(text=f"Seed: {seed}")
        self.fair_prev_label.config(text=f"PrevChain: {prev_chain[:32]}{'...' if len(prev_chain)>32 else ''}")
        self.fair_chain_label.config(text=f"HashChain: {chain[:32]}{'...' if len(chain)>32 else ''}")

    def copy_fairness_info(self):
        seed = self.last_fairness.get('seed') or '-' 
        prev_chain = self.last_fairness.get('prev_chain') or '-'
        chain = self.last_fairness.get('hash_chain') or '-'
        txt = f"Seed={seed}\nPrevChain={prev_chain}\nHashChain={chain}"
        try:
            self.root.clipboard_clear()
            self.root.clipboard_append(txt)
            self.info_label.config(text="公平性信息已复制")
        except Exception:
            self.info_label.config(text="复制失败")
